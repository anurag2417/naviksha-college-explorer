...
export default function Colleges() {
  ...
  const getSafeValue = (college, path, defaultValue = 0) => {
  ...
  const calculateAverage = (colleges, path) => {
    if (!colleges.length) return 0;
    
    const validColleges = colleges.filter(college => 
      getSafeValue(college, path, null) !== null
    );
    
    if (!validColleges.length) return 0;
    
    const sum = validColleges.reduce((acc, college) => 
      acc + getSafeValue(college, path, 0), 0
    );
  ...
  const getRankingBadge = (ranking: number) => {
    if (!ranking) return "bg-gradient-to-r from-gray-500/20 to-gray-700/20 text-gray-400";
    if (ranking <= 10) return "bg-gradient-to-r from-yellow-500/20 to-amber-500/20 text-yellow-400";
    if (ranking <= 50) return "bg-gradient-to-r from-purple-500/20 to-pink-500/20 text-purple-400";
    if (ranking <= 100) return "bg-gradient-to-r from-blue-500/20 to-cyan-500/20 text-blue-400";
  ...
  const avgPackage = calculateAverage(filteredColleges, "placement.averagePackage");
  const avgFees = calculateAverage(filteredColleges, "fees.annual");
  const avgRating = calculateAverage(filteredColleges, "rating");
  const avgPlacementPercentage = calculateAverage(filteredColleges, "placement.percentage");

  // Get highest package safely
  const highestPackage = filteredColleges.length > 0 
    ? Math.max(...filteredColleges.map(college => getSafeValue(college, "placement.highestPackage", 0)))
    : 0;
    ...
    <main className="min-h-screen bg-gradient-to-br from-[#0a0a0a] via-[#0f0f0f] to-[#050505] text-[#f3f4f6] px-4 sm:px-6 py-16 sm:py-20 selection:bg-[#4f46e5] selection:text-white overflow-hidden">
        ...
        <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-6">
          <div className="flex-1">
            ...
            <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold tracking-tight bg-gradient-to-r from-[#f3f4f6] via-[#e5e7eb] to-[#9ca3af] bg-clip-text text-transparent mb-3">
              Top {stream && streamDisplayNames[stream]} {type && typeDisplayNames[type]} Colleges
            </h1>
            
            <p className="text-[#9ca3af] text-sm sm:text-base lg:text-lg font-light max-w-3xl leading-relaxed">
              Discover and compare leading institutions with detailed rankings, placement records, fees, and admission criteria
            </p>
          </div>
          ...
          <div className="flex flex-col sm:flex-row lg:flex-col gap-4">
            <div className="px-4 py-3 rounded-xl bg-[#1f2937] border border-[#374151] text-[#9ca3af] text-sm">
              ...
              <div>
                <span className="font-bold text-xl text-[#f3f4f6]">{filteredColleges.length}</span>
                <span className="ml-1.5">College{filteredColleges.length !== 1 ? 's' : ''}</span>
              </div>
            </div>
      ...
      <div className="relative max-w-7xl mx-auto mb-10 animate-fade-in-delay">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            {
              label: "Top Package",
              value: highestPackage > 0 ? formatCurrency(highestPackage) : "N/A",
              icon: "💰",
            ...
            {
              label: "Avg. Placement %",
              value: avgPlacementPercentage > 0 ? `${Math.round(avgPlacementPercentage)}%` : "N/A",
              icon: "📈",
      ...
      <div className="relative max-w-7xl mx-auto grid gap-4 sm:gap-6">
        {filteredColleges.map((college, index) => {
          ...
          const safeAvgPackage = getSafeValue(college, "placement.averagePackage", 0);
          const safeAnnualFees = getSafeValue(college, "fees.annual", 0);
          const safePlacementPercentage = getSafeValue(college, "placement.percentage", 0);
          const safeRanking = getSafeValue(college, "ranking", 9999);
          const safeTopRecruiters = getSafeValue(college, "placement.topRecruiters", []);
          const safeAccreditation = getSafeValue(college, "accreditation", "NA");
          const safeEstablished = getSafeValue(college, "established", "N/A");
          const safeLocation = getSafeValue(college, "location", { city: "N/A", state: "N/A" });
          ...
          return (
            ...
            >
              {/* Gradient overlay */}
              <div className={`absolute inset-0 bg-gradient-to-br ${getGradientForIndex(index)} opacity-0 group-hover:opacity-100 transition-opacity duration-500`} />
              
              {/* Ranking Badge */}
              {safeRanking !== 9999 && (
                <div className={`absolute top-4 right-4 z-10 px-3 py-1.5 rounded-full text-xs font-bold ${getRankingBadge(safeRanking)} border border-white/10`}>
                  #{safeRanking}
                </div>
              )}
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
                      ...
                      <div className="p-3 rounded-xl bg-[#1f2937]/50 border border-[#374151] group-hover:border-[#374151]/70 transition-colors">
                        <div className="flex items-center gap-1.5 mb-1">
                          <span className={`text-lg font-bold ${getRatingColor(safeRating)}`}>
                      ...
                      <div className="p-3 rounded-xl bg-[#1f2937]/50 border border-[#374151] group-hover:border-[#374151]/70 transition-colors">
                        <div className="text-lg font-bold text-emerald-400 mb-1">
                          {safeAvgPackage > 0 ? formatCurrency(safeAvgPackage) : "N/A"}
                        </div>
                        <div className="text-xs text-[#9ca3af]">Avg. Package</div>
                      </div>

                      {/* Annual Fees */}
                      <div className="p-3 rounded-xl bg-[#1f2937]/50 border border-[#374151] group-hover:border-[#374151]/70 transition-colors">
                        <div className="text-lg font-bold text-blue-400 mb-1">
                          {safeAnnualFees > 0 ? formatCurrency(safeAnnualFees) : "N/A"}
                        </div>
                        <div className="text-xs text-[#9ca3af]">Annual Fees</div>
                      </div>

                      {/* Placement % */}
                      <div className="p-3 rounded-xl bg-[#1f2937]/50 border border-[#374151] group-hover:border-[#374151]/70 transition-colors">
                        <div className="text-lg font-bold text-green-400 mb-1">
                          {safePlacementPercentage > 0 ? `${safePlacementPercentage}%` : "N/A"}
                        </div>
                ...
                {safeTopRecruiters.length > 0 && (
                  <div className="mt-6 pt-6 border-t border-[#1f2937] group-hover:border-[#374151] transition-colors">
                    <div className="flex items-center justify-between mb-3">
                      <span className="text-sm font-medium text-[#e5e7eb]">Top Recruiters</span>
                      <span className="text-xs text-[#9ca3af]">
                        {safeTopRecruiters.length} {safeTopRecruiters.length === 1 ? 'company' : 'companies'}
                      </span>
                    </div>
                    <div className="flex flex-wrap gap-2">
                      {safeTopRecruiters.slice(0, 5).map((recruiter, idx) => (
                        <span 
                          key={idx}
        ...
        {filteredColleges.length === 0 && (
          <div className="relative rounded-2xl border border-[#1f2937] bg-gradient-to-br from-[#0f0f0f] to-[#1a1a1a] p-12 sm:p-16 text-center animate-fade-in">
            ...
            <p className="text-[#9ca3af] mb-6 max-w-md mx-auto">
              We couldn't find any colleges matching your selected criteria. Try adjusting your filters or explore other options.
            </p>
        ...
        <div className="relative max-w-7xl mx-auto mt-10 sm:mt-12 pt-8 border-t border-[#1f2937] animate-fade-in-delay-long">
          <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6">
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-sm font-medium text-[#e5e7eb]">Important Information</span>
              </div>
              <p className="text-sm text-[#9ca3af] max-w-2xl">
                All data is collected from official sources and updated regularly. Fees mentioned are approximate annual tuition fees. 
                Placement packages are average figures from the last placement season. Rankings are based on NIRF 2024 where applicable.
              </p>
            </div>
          ...
          <div className="mt-6 pt-6 border-t border-[#1f2937]">
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4 text-xs text-[#6b7280]">
              <div>
                Showing <span className="font-semibold text-[#f3f4f6]">{filteredColleges.length}</span> out of <span className="font-semibold text-[#f3f4f6]">{colleges.length}</span> total colleges
              </div>
              <div className="flex items-center gap-4">
                <span>Need help? <a href="#" className="text-[#4f46e5] hover:text-[#6366f1] transition-colors">Contact our experts</a></span>
                <span>•</span>
